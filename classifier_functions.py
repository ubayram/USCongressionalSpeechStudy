...
def getClassifierParams(training_matrix, validation_matrix, training_labels, validation_labels):
    ...
    merged_labels = training_labels + validation_labels

    print('Merged matrix size ' + str(np.shape(merged_matrix)))
    print('Merged labels ' + str(len(merged_labels)))
    grid = findBestSVMParameters(merged_matrix, merged_labels)

    return grid

def findBestSVMParameters(train_matrix, train_labels):

    gamma_ = [pow(2, -8), pow(2, -4), 1, pow(2, 4), pow(2, 8)]
    cost_range = [pow(2, -10), pow(2, -5), pow(2, 0), pow(2, 5), pow(2, 10)]

    kernels = ['linear', 'rbf'] # got rid of poly, it's never poly , 'sigmoid'
    param_grid = {'C': cost_range, 'kernel': kernels, 'gamma': gamma_, 'shrinking':[False]}

    grid = GridSearchCV(SVC(), param_grid=param_grid, scoring= 'accuracy', cv=4, verbose = True, n_jobs = -1) # parallel search on
    grid.fit(train_matrix, train_labels)

    print("The best SVM parameters are %s with a score of %0.2f" % (grid.best_params_, grid.best_score_))

    return grid

# inside, save the trained model to the corresponding folder - might be needed in the future
def trainTheClassifier(training_matrix, training_labels, grid):
    ...
    classifier.fit(training_matrix.toarray(), training_labels)

    return classifier

def classifyFeatures(input_matrix, trained_model):

    probas_ = trained_model.predict_proba(input_matrix)
